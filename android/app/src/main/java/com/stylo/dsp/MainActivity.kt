...
class MainActivity : Activity() {
    ...
    private fun loadPreset(){val param=prefs.getBoolean("param",false);model.parametricMode=param;model.bypass=prefs.getBoolean("bypass",false);val list=model.active;list.forEachIndexed{i,b->{if(prefs.contains("g$i")){b.gain=prefs.getFloat("g$i",0f);b.q=prefs.getFloat("q$i",1f);b.freq=prefs.getFloat("f$i",b.freq.toFloat()).toDouble();b.type=EqModel.Type.values()[prefs.getInt("t$i",0).coerceIn(0,EqModel.Type.values().lastIndex)]}}}}
    ...
    private inner class EqView:View(this){private val p=Paint(Paint.ANTI_ALIAS_FLAG);private val path=Path();private val spectrum=FloatArray(1024);private var drag=false;private var pinch=false;private var pinchStart=0f;private var qStart=1f
        ...
        private fun distance(e:MotionEvent)=if(e.pointerCount<2)1f{val dx=e.getX(0)-e.getX(1);val dy=e.getY(0)-e.getY(1);sqrt(dx*dx+dy*dy)}
    }
}
