...
class MainActivity : Activity() {
    private val model=EqModel();private lateinit var graph:EqView;private var recorder:AudioRecord?=null;private var audioThread:Thread?=null;@Volatile private var running=false
    private var player:MediaPlayer?=null;private var playerEq:Equalizer?=null;private var seek:SeekBar?=null;private var timeText:TextView?=null;private var titleText:TextView?=null
    private val prefs by lazy{getSharedPreferences("stylo_eq",0)}
    override fun onCreate(state:Bundle?){super.onCreate(state);loadPreset();buildUi();if(checkSelfPermission(Manifest.permission.RECORD_AUDIO)!=PackageManager.PERMISSION_GRANTED)requestPermissions(arrayOf(Manifest.permission.RECORD_AUDIO),10)else startAnalyzer()}
    private fun buildUi(){val root=LinearLayout(this).apply{orientation=LinearLayout.VERTICAL;setBackgroundColor(0xff090a0c.toInt())};graph=EqView();root.addView(graph,LinearLayout.LayoutParams(-1,0,1f));titleText=TextView(this).apply{text="Sin archivo · STYLO EQ";setTextColor(0xffb8c3c8.toInt());textSize=13f;setPadding(18,2,18,0)};root.addView(titleText);seek=SeekBar(this).apply{max=1000;setPadding(18,0,18,0);setOnSeekBarChangeListener(object:SeekBar.OnSeekBarChangeListener{override fun onProgressChanged(s:SeekBar?,p:Int,from:Boolean){if(from)player?.let{if(it.duration>0)it.seekTo(it.duration*p/1000);updateTime()}};override fun onStartTrackingTouch(s:SeekBar?){};override fun onStopTrackingTouch(s:SeekBar?){} })};root.addView(seek,LinearLayout.LayoutParams(-1,42));timeText=TextView(this).apply{text="00:00 / 00:00";gravity=Gravity.CENTER;setTextColor(0xffd9e4e8.toInt());textSize=12f};root.addView(timeText,LinearLayout.LayoutParams(-1,25))
        val row=LinearLayout(this).apply{gravity=Gravity.CENTER};fun b(t:String,a:()->Unit)=Button(this).apply{text=t;setOnClickListener{a()}};row.addView(b("📂"){openAudio()});row.addView(b("⏮"){player?.seekTo(max(0,(player?.currentPosition?:0)-10000))});row.addView(b("▶/⏸"){togglePlay()});row.addView(b("⏭"){player?.seekTo(min(player?.duration?:0,(player?.currentPosition?:0)+10000))});row.addView(b("A/B"){model.ab=!model.ab;graph.invalidate();applyEq()});root.addView(row,LinearLayout.LayoutParams(-1,52))
        val eq=LinearLayout(this).apply{gravity=Gravity.CENTER};eq.addView(b("31-BAND"){model.parametricMode=false;model.selected=15;graph.invalidate();applyEq()});eq.addView(b("PARAM 8"){model.parametricMode=true;model.selected=0;graph.invalidate();applyEq()});eq.addView(b("TYPE"){cycleType()});eq.addView(b("BYPASS"){model.bypass=!model.bypass;graph.invalidate();applyEq()});eq.addView(b("RESET"){model.active.forEach{it.gain=0f};graph.invalidate();applyEq()});root.addView(eq,LinearLayout.LayoutParams(-1,52))
        val p=LinearLayout(this).apply{gravity=Gravity.CENTER};p.addView(b("SAVE"){savePreset()});p.addView(b("LOAD"){loadPreset();graph.invalidate();applyEq()});p.addView(b("FLAT"){model.active.forEach{it.gain=0f;it.type=EqModel.Type.PEAK};graph.invalidate();applyEq()});root.addView(p,LinearLayout.LayoutParams(-1,48));setContentView(root)}
    private fun cycleType(){val b=model.active[model.selected];val types=EqModel.Type.values();b.type=types[(b.type.ordinal+1)%types.size];graph.invalidate();applyEq()}
    ...
    private fun loadPreset(){model.parametricMode=prefs.getBoolean("param",false);model.bypass=prefs.getBoolean("bypass",false);model.active.forEachIndexed{i,b->{if(prefs.contains("g$i")){b.gain=prefs.getFloat("g$i",0f);b.q=prefs.getFloat("q$i",1f);b.freq=prefs.getFloat("f$i",b.freq.toFloat()).toDouble();b.type=EqModel.Type.values()[prefs.getInt("t$i",0).coerceIn(0,EqModel.Type.values().lastIndex)]}}}}
    ...
    private inner class EqView:View(this){private val p=Paint(Paint.ANTI_ALIAS_FLAG);private val path=Path();private val spectrum=FloatArray(1024);private var drag=false;private var pinch=false;private var pinchStart=0f;private var qStart=1f
