...
class MainActivity : Activity() {
    ...
    private fun buildUi(){
        ...
        val row=LinearLayout(this).apply{gravity=Gravity.CENTER};fun b(t:String,a:()->Unit)=Button(this).apply{text=t;setOnClickListener{a()}};row.addView(b("📂"){openAudio()});row.addView(b("⏮"){player?.seekTo(max(0,(player?.currentPosition?:0)-10000))});row.addView(b("▶/⏸"){togglePlay()});row.addView(b("⏭"){player?.seekTo(min(player?.duration?:0,(player?.currentPosition?:0)+10000))});row.addView(b("A/B"){model.ab=!model.ab;refresh()});root.addView(row,LinearLayout.LayoutParams(-1,52))
        val eq=LinearLayout(this).apply{gravity=Gravity.CENTER};eq.addView(b("31-BAND"){model.parametricMode=false;model.selected=15;refresh()});eq.addView(b("PARAM 8"){model.parametricMode=true;model.selected=0;refresh()});eq.addView(b("TYPE"){cycleType();refresh()});eq.addView(b("BYPASS"){model.bypass=!model.bypass;refresh()});eq.addView(b("RESET"){resetActive();refresh()});root.addView(eq,LinearLayout.LayoutParams(-1,52))
        val p=LinearLayout(this).apply{gravity=Gravity.CENTER};p.addView(b("SAVE"){savePreset()});p.addView(b("LOAD"){loadPreset();refresh()});p.addView(b("FLAT"){resetActive();refresh()});root.addView(p,LinearLayout.LayoutParams(-1,48));setContentView(root)
    }
    ...
    private fun loadPreset(){model.parametricMode=prefs.getBoolean("param",false);model.bypass=prefs.getBoolean("bypass",false);val list=model.active;for(i in list.indices){val b=list[i];if(prefs.contains("g$i")){b.gain=prefs.getFloat("g$i",0f);b.q=prefs.getFloat("q$i",1f);b.freq=prefs.getFloat("f$i",b.freq.toFloat()).toDouble();val ti=prefs.getInt("t$i",0).coerceIn(0,EqModel.Type.values().lastIndex);b.type=EqModel.Type.values()[ti]}}}
    ...
    private inner class EqView:View(this){
        ...
        override fun onDraw(c:Canvas){c.drawColor(0xff090a0c.toInt());val w=width.toFloat();val h=height.toFloat();p.style=Paint.Style.STROKE;p.strokeWidth=1f;p.color=0x30363c42;for(i in 0..10)c.drawLine(0f,h*.08f+i*h*.80f/10,w,h*.08f+i*h*.80f/10,p);for(f in doubleArrayOf(20.0,50.0,100.0,500.0,1000.0,5000.0,10000.0,20000.0)){val x=fToX(f);c.drawLine(x,h*.06f,x,h*.90f,p)}
            p.color=0xaa67ddf2.toInt();p.strokeWidth=1.5f;spectrumPath.reset();var started=false;for(i in 1 until spectrum.size){val f=i.toDouble()/spectrum.size*22050;if(f<20||f>20000)continue;val x=fToX(f);val db=spectrum[i].coerceIn(-80f,0f);val y=h*.88f-(db+80f)/80f*h*.76f;if(!started){spectrumPath.moveTo(x,y);started=true}else spectrumPath.lineTo(x,y)};c.drawPath(spectrumPath,p)
            p.color=0xffe9f4f7.toInt();p.strokeWidth=3f;eqPath.reset();for(x in 0..w.toInt() step 4){val y=gToY(model.responseDb(xToF(x.toFloat())));if(x==0)eqPath.moveTo(0f,y)else eqPath.lineTo(x.toFloat(),y)};c.drawPath(eqPath,p);p.style=Paint.Style.FILL;model.active.forEachIndexed{idx,b->p.color=if(idx==model.selected)0xff7de7ff.toInt()else 0xff789099.toInt();c.drawCircle(fToX(b.freq),gToY(b.gain.toDouble()),if(idx==model.selected)9f else 4f,p)}
