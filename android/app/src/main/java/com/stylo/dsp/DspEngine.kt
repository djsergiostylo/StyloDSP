...
/** Realtime FFT primitives. process() reuses all buffers and performs no heap allocation. */
class Fft(private val size: Int) {
    private val real = DoubleArray(size)
    private val imag = DoubleArray(size)
    private val window = DoubleArray(size) { i ->
        0.5 - 0.5 * cos(2.0 * PI * i / (size - 1).toDouble())
    }
    ...
    init {
        ...
        for (i in twiddleCos.indices) {
            val angle = -2.0 * PI * i.toDouble() / size.toDouble()
            twiddleCos[i] = cos(angle)
            twiddleSin[i] = sin(angle)
        }
    }
    ...
    fun process(input: ShortArray, count: Int, magnitudesDb: FloatArray) {
        ...
        for (i in 0 until n) {
            val magnitude = sqrt(real[i] * real[i] + imag[i] * imag[i]) / size.toDouble()
            magnitudesDb[i] = (20.0 * log10(maxOf(1e-7, magnitude))).toFloat()
        }
    }
}

enum class FilterType {
    PEAK, LOW_SHELF, HIGH_SHELF, LOW_PASS, HIGH_PASS, NOTCH, BAND_PASS, ALL_PASS, TILT
}
...
class Biquad(private val sampleRate: Double) {
    ...
    fun configure(band: EqBand) {
        ...
        val frequency = band.frequency.coerceIn(10.0, sampleRate * 0.49)
        ...
        when (band.type) {
            ...
            FilterType.NOTCH -> { B0 = 1.0; B1 = -2 * c; B2 = 1.0; A0 = 1 + alpha; A1 = -2 * c; A2 = 1 - alpha }
...
class EqBank(private val sampleRate: Double, initialBands: List<EqBand> = emptyList()) {
    private var bands: List<EqBand> = initialBands.map { it.copy() }
    private var filters: Array<Biquad> = Array(bands.size) { Biquad(sampleRate) }

    fun setBands(newBands: List<EqBand>) {
        if (newBands.size != filters.size) {
            filters = Array(newBands.size) { Biquad(sampleRate) }
        }
        newBands.forEachIndexed { index, band -> filters[index].configure(band) }
        bands = newBands.map { it.copy() }
    }

    fun configureAll() { bands.forEachIndexed { index, band -> filters[index].configure(band) } }
    fun process(x: Double): Double { var y = x; for (filter in filters) y = filter.process(y); return y }
