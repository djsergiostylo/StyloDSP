...
class Fft(private val size: Int) {
    ...
    init {
        ...
        var j = 0
        for (i in 1 until size) { var bit = size shr 1; while (j and bit != 0) { j = j xor bit; bit = bit shr 1 }; j = j xor bit; bitReverse[i] = j }
        for (i in twiddleCos.indices) { val a = -2.0 * PI * i / size; twiddleCos[i] = cos(a); twiddleSin[i] = sin(a) }
    }
    fun process(input: ShortArray, count: Int, magnitudesDb: FloatArray) {
        val safeCount = count.coerceIn(0, size)
        for (i in 0 until size) { val x = if (i < safeCount) input[i].toDouble() / 32768.0 else 0.0; real[bitReverse[i]] = x * window[i]; imag[bitReverse[i]] = 0.0 }
        var len = 2
        while (len <= size) { val half = len shr 1; val step = size / len; var base = 0; while (base < size) { var j = 0; var tw = 0; while (j < half) { val wr = twiddleCos[tw]; val wi = twiddleSin[tw]; val i0 = base+j; val i1=i0+half; val tr=wr*real[i1]-wi*imag[i1]; val ti=wr*imag[i1]+wi*real[i1]; val r0=real[i0]; val im0=imag[i0]; real[i1]=r0-tr; imag[i1]=im0-ti; real[i0]=r0+tr; imag[i0]=im0+ti; j++; tw+=step }; base+=len }; len=len shl 1 }
        val n=minOf(magnitudesDb.size,size/2)
        for(i in 0 until n){val mag=sqrt(real[i]*real[i]+imag[i]*imag[i])/size;magnitudesDb[i]=(20.0*log10(maxOf(1e-7,mag))).toFloat()}
    }
}
...
class Biquad(private val sampleRate:Double){
    ...
    fun configure(band:EqBand){
        ...
        val f=band.frequency.coerceIn(10.0,sampleRate*.49);val q=band.q.coerceIn(.1,18.0);val A=exp(ln(10.0)*band.gainDb/40.0);val w0=2.0*PI*f/sampleRate;val c=cos(w0);val s=sin(w0);val alpha=s/(2.0*q)
        ...
        when(band.type){
            ...
            FilterType.BAND_PASS->{B0=alpha;B1=0.0;B2=-alpha;A0=1+alpha;A1=-2*c;A2=1-alpha}
...
/** Dynamic filter bank. Coefficients are updated in place so UI parameter edits do not rebuild filters or reset state. */
class EqBank(private val sampleRate:Double, initialBands:List<EqBand> = emptyList()){
    ...
    fun setBands(newBands:List<EqBand>){
        if(newBands.size!=filters.size){filters=Array(newBands.size){Biquad(sampleRate)};newBands.forEachIndexed{i,b->filters[i].configure(b)}}
        else newBands.forEachIndexed{i,b->filters[i].configure(b)}
...
class SafetyLimiter(private val drive:Double=1.0){
    fun process(x:Double):Double{val y=x*drive;return if(y>1.0)1.0 else if(y< -1.0)-1.0 else y}
}
