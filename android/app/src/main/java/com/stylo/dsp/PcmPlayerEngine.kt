...
class PcmPlayerEngine(private val context: Context, private val onState: (Int, Int, Boolean) -> Unit) {
    ...
    fun seekTo(ms: Int) {
        seekRequestMs = ms.coerceIn(0, durationMs.coerceAtLeast(0))
        if (worker?.isAlive != true) startWorker()
        emitState()
    }
    ...
    private fun decodeLoop() {
        ...
        try {
            ...
            durationMs = if (format.containsKey(MediaFormat.KEY_DURATION)) {
                (format.getLong(MediaFormat.KEY_DURATION) / 1000L).toInt()
            } else 0
            ...
            while (!stopRequested.get() && !outputDone) {
                seekRequestMs?.let { target ->
                    ...
                    audioTrack?.pause()
                    audioTrack?.flush()
                    if (playing.get()) audioTrack?.play()
                    emitState()
                }

                val currentBands = bands
                if (lastBands != currentBands) {
                    lastBands = currentBands.map { it.copy() }
                    processors.forEach { bank ->
                        bank.setBands(lastBands)
                        bank.reset()
                    }
                }
                    ...
                    if (out != null && info.size > 0) {
                        ...
                        if (!bypass) {
                            for (i in 0 until sampleCount) {
                                ...
                                scratch[i] = (processors[ch].process(x).coerceIn(-1.0, 1.0) * 32767.0).toInt().toShort()
                            }
                        }
