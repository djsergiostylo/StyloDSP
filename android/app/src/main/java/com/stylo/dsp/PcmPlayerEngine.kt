...
class PcmPlayerEngine(
    ...
    private val onState: (Int, Int, Boolean) -> Unit,
    private val onSpectrum: ((FloatArray) -> Unit)? = null
) {
    ...
    fun setVolume(value: Float) { volume = value.coerceIn(0f, 1f); audioTrack?.setVolume(volume) }
    fun setLoop(value: Boolean) { loop = value }
    fun playPause() {
        if (uri == null) return
        if (playing.get()) {
            playing.set(false)
    ...
    private fun decodeLoop() {
        ...
        try {
            ...
            for (i in 0 until extractor.trackCount) {
                ...
                if (format.getString(MediaFormat.KEY_MIME)?.startsWith("audio/") == true) { trackIndex = i; break }
            }
            ...
            val limiter = SafetyLimiter()
            val info = MediaCodec.BufferInfo()

            while (!stopRequested.get() && !outputDone) {
                seekRequestMs?.let { target ->
                    seekRequestMs = null
                    extractor.seekTo(target.toLong() * 1000L, MediaExtractor.SEEK_TO_CLOSEST_SYNC)
                    codec.flush()
                    inputDone = false
                    outputDone = false
                    positionMs = target
                    processors.forEach { it.reset() }
                    audioTrack?.pause(); audioTrack?.flush(); if (playing.get()) audioTrack?.play()
                    fftFill = 0
                    emitState()
                }
                ...
                if (appliedVersion != version) {
                    val snapshot = bands
                    processors.forEach { bank -> bank.setBands(snapshot); bank.reset() }
                ...
                if (outIndex >= 0) {
                    ...
                    if (out != null && info.size > 0) {
                        ...
                        val view = out.duplicate().order(ByteOrder.LITTLE_ENDIAN)
                        view.position(info.offset); view.limit(info.offset + info.size)
                        view.asShortBuffer().get(scratch, 0, sampleCount)

                        if (!bypass) {
                            for (i in 0 until sampleCount) {
                                val ch = i % channels
                                val x = scratch[i].toDouble() / 32768.0
                                scratch[i] = (limiter.process(processors[ch].process(x)).coerceIn(-1.0, 1.0) * 32767.0).toInt().toShort()
                            }
                        }

                        if (fft != null && fftInput != null && mags != null) {
                            var i = 0
                            while (i < sampleCount && fftFill < fftInput.size) {
                                val mono: Short = if (channels == 1) {
                                    scratch[i]
                                } else if (i + 1 < sampleCount) {
                                    ((scratch[i].toInt() + scratch[i + 1].toInt()) / 2).toShort()
                                } else {
                                    scratch[i]
                                }
                                fftInput[fftFill++] = mono
                                i += channels
                            }
                            val now = System.nanoTime()
                            if (fftFill == fftInput.size && now - lastSpectrumNs >= 33_000_000L) {
                                fft.process(fftInput, fftInput.size, mags)
                                val snapshot = mags.copyOf()
                                handler.post { onSpectrum?.invoke(snapshot) }
                                lastSpectrumNs = now
                                fftFill = 0
                            } else if (fftFill == fftInput.size) {
                                fftFill = 0
                            }
                        }

                        audioTrack?.write(scratch, 0, sampleCount, AudioTrack.WRITE_BLOCKING)
                        positionMs = (info.presentationTimeUs / 1000L).toInt()
                        emitState()
                    }
                    ...
                    if ((info.flags and MediaCodec.BUFFER_FLAG_END_OF_STREAM) != 0) {
                        if (loop && !stopRequested.get()) {
                            ...
                            codec.flush(); inputDone = false; outputDone = false; positionMs = 0
                            processors.forEach { it.reset() }
                            audioTrack?.pause(); audioTrack?.flush(); if (playing.get()) audioTrack?.play()
                            fftFill = 0; emitState()
                        } else {
                            playing.set(false); outputDone = true
                        }
                    }
                }
            ...
            runCatching { audioTrack?.stop() }; runCatching { audioTrack?.release() }
            audioTrack = null
            emitState()
        }
    }
