...
class PcmPlayerEngine(private val context: Context, private val onState: (Int, Int, Boolean) -> Unit, private val onSpectrum: ((FloatArray) -> Unit)? = null) {
    ...
    private fun decodeLoop() {
        ...
        try {
            ...
            var lastBands: List<EqBand> = emptyList(); var inputDone = false; var outputDone = false; var scratch = ShortArray(4096)
            val fft = if (onSpectrum != null) Fft(2048) else null
            val fftInput = if (onSpectrum != null) ShortArray(2048) else null
            val mags = if (onSpectrum != null) FloatArray(1024) else null
            var fftFill = 0; var lastSpectrumNs = 0L
            ...
            while (!stopRequested.get() && !outputDone) {
                ...
                val currentBands = bands
                if (lastBands != currentBands) { lastBands = currentBands.map { it.copy() }; processors.forEach { bank -> bank.setBands(lastBands); bank.reset() } }
                if (!inputDone) {
                    val inputIndex = codec.dequeueInputBuffer(10_000)
                    if (inputIndex >= 0) { val input = codec.getInputBuffer(inputIndex) ?: continue; input.clear(); val size = extractor.readSampleData(input, 0); if (size < 0) { codec.queueInputBuffer(inputIndex, 0, 0, 0L, MediaCodec.BUFFER_FLAG_END_OF_STREAM); inputDone = true } else { codec.queueInputBuffer(inputIndex, 0, size, extractor.sampleTime, 0); extractor.advance() } }
                }
                ...
                if (outIndex >= 0) {
                    ...
                    if (out != null && info.size > 0) {
                        ...
                        val view = out.duplicate().order(ByteOrder.LITTLE_ENDIAN); view.position(info.offset); view.limit(info.offset + info.size); view.asShortBuffer().get(scratch, 0, sampleCount)
                        if (!bypass) for (i in 0 until sampleCount) { val ch = i % channels; val x = scratch[i] / 32768.0; scratch[i] = (processors[ch].process(x).coerceIn(-1.0, 1.0) * 32767.0).toInt().toShort() }
                        if (fft != null && fftInput != null && mags != null) {
                            var i = 0
                            while (i < sampleCount && fftFill < fftInput.size) { val mono = if (channels == 1) scratch[i] else ((scratch[i].toInt() + scratch.getOrElse(i + 1) { scratch[i].toInt() }) / 2).toShort(); fftInput[fftFill++] = mono; i += channels }
                            val now = System.nanoTime()
                            if (fftFill == fftInput.size && now - lastSpectrumNs >= 33_000_000L) { fft.process(fftInput, fftInput.size, mags); val snapshot = mags.copyOf(); handler.post { onSpectrum?.invoke(snapshot) }; lastSpectrumNs = now; fftFill = 0 } else if (fftFill == fftInput.size) fftFill = 0
                        }
                    ...
                    if (info.flags and MediaCodec.BUFFER_FLAG_END_OF_STREAM != 0) {
                        if (loop && !stopRequested.get()) { extractor.seekTo(0L, MediaExtractor.SEEK_TO_CLOSEST_SYNC); codec.flush(); inputDone = false; positionMs = 0; processors.forEach { it.reset() }; audioTrack?.pause(); audioTrack?.flush(); if (playing.get()) audioTrack?.play(); fftFill = 0; emitState() } else { playing.set(false); outputDone = true }
                    }
                }
                if (playing.get()) audioTrack?.play() else audioTrack?.pause()
            }
        } catch (_: Throwable) { playing.set(false) }
        finally { runCatching { codec?.stop() }; runCatching { codec?.release() }; runCatching { extractor.release() }; runCatching { audioTrack?.stop() }; runCatching { audioTrack?.release() }; audioTrack = null; emitState() }
    }
